---
title: Ambassador program — open items
project: ambassador-program
---

Pulled from `overview.md`'s open-questions and evaluation sections.

- [x] Who is an "ambassador" — resolved 2026-08-19: visible community
      leads, people who already show up to events and do the work; see
      overview.md.
- [ ] What does "compare notes and share solutions" run on — a mailing
      list, recurring call, or the venue [[community-forum-platform]] is
      already exploring? Check for overlap before standing up a separate
      channel.
- [ ] Reconcile "Speaker Travel Program" naming/scope against the
      existing Travel Assistance Committee — resolved in principle
      (own effort, donor-funded, volunteer-selected) but not yet raised
      with the draft's author or the PMC.
- [ ] Confirm whether `community.apache.org/metrics/` is the intended
      eventual path for the metrics dashboard, or just a placeholder in
      the draft — current reality is externally hosted at
      <https://boxofclue.com/comdev-metrics/>.
- [ ] Who provisions/maintains the ComDev-owned machine the metrics
      dashboard is meant to move onto.
- [ ] De-duplicate the Good First Issues aggregator build against
      [[university-outreach]]'s template layer — one build should serve
      both threads, not two.
- [ ] Name who actually builds the "equip" toolkit prerequisite (metrics
      dashboard, onboarding/welcome-email templates, escalation path)
      and by when — flagged in the draft, not yet assigned.
